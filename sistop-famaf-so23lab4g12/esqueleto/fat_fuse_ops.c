...
void fat_fuse_log_activity(char *operation_type, fat_file target_file){
    ...
    fat_tree_node log_n = fat_tree_node_search(vol->file_tree, LOG_FILE);
    fat_file log_f= fat_tree_get_file(log_n);
    fat_file log_p = fat_tree_get_parent(log_n);
    ...
    fat_file_pwrite(log_f, buf,strlen(buf)+1, log_f->dentry->file_size, log_p);
}
...
static void fat_fuse_log_init(){
    ...
    if(log_n==NULL){
        if (fat_fuse_mknod(LOG_FILE,0,0)!=0){
            perror("Error creating log file");
            return;
        }
        log_n = fat_tree_node_search(vol->file_tree, LOG_FILE);
        fat_file log_f= fat_tree_get_file(log_n);
        fat_fuse_log_activity("INIT", log_f);
    }
}
...
static int is_fs_log(fat_file file) {
    return (is_log_filepath(file->filepath));
}
...
int fat_fuse_unlink(const char *path) {
    ...
    fat_volume vol = get_fat_volume();
    fat_file file = NULL, parent = NULL;
    fat_tree_node file_n = fat_tree_node_search(vol->file_tree, path);
    if (file_n == NULL || errno != 0) {
        errno = ENOENT;
        return -errno;
    }
    file = fat_tree_get_file(file_n);
    if (fat_file_is_directory(file)) {
        errno = EISDIR;
        return -errno;
    }

    parent = fat_tree_get_parent(file_n);
...
int fat_fuse_rmdir(const char *path) {
    ...
    fat_file dir = fat_tree_get_file(file_n);
    if (!fat_file_is_directory(dir)) {
        errno = ENOTDIR;
        return -errno;
    }
    ...
    fat_file parent = fat_tree_get_parent(file_n);
